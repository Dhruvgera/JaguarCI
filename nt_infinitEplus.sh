...
export KBUILD_BUILD_USER="infinitEplus"
...
export RELEASE-BUILD="RELEASE-TYPE"
...
awk -F ')' '{print $2}' | awk '{print tolower($1)}')"
export ZIPNAME="${KERNELNAME}-${MAKE_TYPE}-${RELEASE_TYPE}-$(date +%Y%m%d-%H%M).zip"
...
if [[ ${success} == true ]]; then
    ...
    transfer "${FINAL_ZIP}";
source common
message="Jaguar Kernel Build By @infinitEplus completed."
